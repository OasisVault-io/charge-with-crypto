...
async function resolveCheckoutFromMerchant({ merchant, config, referenceId, planId = '' }) {
  ...
  if (merchant.webhookUrl.startsWith('mock://')) {
    ...
    return {
      ...
      acceptedAssets: selectedPlan?.acceptedAssets || merchant.defaultAcceptedAssets || ['USDC', 'USDT'],
      enabledChains: selectedPlan?.enabledChains || merchant.enabledChains || Object.keys(config.chains)
    };
  }
