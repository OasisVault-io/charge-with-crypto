...
async function createQuote(store, priceService, config, { checkoutId, chain, asset, fiatCurrency = 'USD', fiatAmount }) {
  ...
  const expiresAt = quoted.source === 'fixed_peg' ? null : addSeconds(issuedAt, config.quoteExpirySeconds);
...
module.exports = { createQuote, getActiveQuote, getActiveQuotesForCheckout, getQuoteById, getLatestQuoteForSelection };
