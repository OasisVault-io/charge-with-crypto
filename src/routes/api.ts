...
async function createCheckoutResponse({ store, config, priceService, manualPaymentService, bitcoinAddressService, body }) {
...
async function handleApi(req, res, ctx) {
  const { store, config, providers, priceService, balanceService, manualPaymentService, bitcoinAddressService } = ctx;
  ...
  if (req.method === 'POST' && path === '/api/merchants') {
    ...
    return sendJson(res, 201, { merchant: redactMerchant(store.insert('merchants', merchant), { includeBitcoinXpub: true }) });
  }
  ...
  if (req.method === 'PATCH' && merchantMatch) {
    ...
    return sendJson(res, 200, { merchant: redactMerchant(store.update('merchants', merchant.id, patch), { includeBitcoinXpub: true }) });
  }
