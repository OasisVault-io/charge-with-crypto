...
const { BitcoinVerifier } = require('./services/bitcoinVerifier');
...
const { BitcoinManualPaymentService } = require('./services/bitcoinManualPaymentService');
const { CompositeManualPaymentService } = require('./services/compositeManualPaymentService');
const { BitcoinAddressService } = require('./services/bitcoinAddressService');
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if (req.url === '/' || req.url.startsWith('/?')) {
        if (sendFile(res, path.join(clientDistDir, 'index.html'))) return;
        sendText(res, 503, 'client build not found; run npm run build or npm run dev');
        return;
      }

      const filePath = path.join(clientDistDir, req.url.split('?')[0]);
      if (filePath.startsWith(clientDistDir) && sendFile(res, filePath)) return;
