...
function promptSecret(prompt: string): Promise<string> {
  if (!process.stdin.isTTY)
    fail('Set MANUAL_PAY_MNEMONIC in env when running without a TTY.')
...
function readJsonRow(db: Database.Database, table: string, id: string) {
  const row = db.prepare(`select json from ${table} where id = ?`).get(id) as
    | { json?: string }
    | undefined
  return row?.json ? (JSON.parse(row.json) as StoredCheckout) : null
}
...
function resolveCheckoutContext(
  ...
  const derivationIndex =
    checkout?.manualPayment?.evm?.derivationIndex ??
    checkout?.manualPayment?.derivationIndex
  if (!Number.isInteger(derivationIndex))
    fail(
      `Checkout ${checkoutId} does not include a manual-payment derivation index.`,
    )

  const chain = String(
    overrides.chain ||
      checkout?.manualPayment?.detectedChain ||
      checkout?.paidChain ||
      '',
  )
    .trim()
  ...
  if (!chain)
    fail(
      `Checkout ${checkoutId} does not have a detected chain yet. Pass --chain base or --chain ethereum.`,
    )

  const token = String(
    overrides.token ||
      checkout?.manualPayment?.detectedAsset ||
      checkout?.paidAsset ||
      '',
  )
    .trim()
  ...
  if (!token)
    fail(
      `Checkout ${checkoutId} does not have a detected token yet. Pass --token USDC or --token USDT.`,
    )

  const toAddress = String(
    overrides.to || checkout?.recipientByChain?.[chain] || '',
  ).trim()
  if (!toAddress)
    fail(
      `Checkout ${checkoutId} does not have a treasury address for ${chain}. Pass --to <address>.`,
    )

  const depositAddress = String(checkout?.manualPayment?.address || '').trim()
  if (!depositAddress)
    fail(`Checkout ${checkoutId} does not include a manual deposit address.`)
...
async function deriveChildWallet(
  mnemonic: string,
...
async function main() {
  ...
  const dataDir = path.resolve(
    projectRoot,
    String(args['data-dir'] || process.env.DATA_DIR || './data'),
  )
  ...
  const chainsConfig = JSON.parse(
    fs.readFileSync(path.join(projectRoot, 'config', 'chains.json'), 'utf8'),
  ) as ChainsConfig
  ...
  if (!tokenAddress)
    fail(
      `No token contract configured for ${context.token} on ${context.chain}.`,
    )
  ...
  const derivationPath = String(
    process.env.MANUAL_PAYMENT_DERIVATION_PATH || "m/44'/60'/0'/0",
  ).trim()
  const { account } = await deriveChildWallet(
    mnemonic,
    derivationPath,
    context.derivationIndex,
  )
  ...
  if (tokenBalance <= 0n)
    fail(`No ${context.token} balance found on ${context.depositAddress}.`)
  ...
  if (amountBaseUnits > tokenBalance)
    fail(
      `Requested amount exceeds balance. Balance is ${tokenBalance.toString()} base units.`,
    )
  ...
  const sponsorAccount = sponsorPrivateKey
    ? privateKeyToAccount(
  ...
  if (nativeBalance < requiredNative) {
    if (!sponsorAccount) {
      fail(
        `Not enough native gas on ${context.depositAddress}. Fund the derived deposit address first on ${context.chain}, or set MANUAL_PAYMENT_SWEEP_SPONSOR_PRIVATE_KEY.\nRequired (est): ${requiredNative.toString()} wei\nCurrent:        ${nativeBalance.toString()} wei`,
      )
    }
    ...
    console.log(
      `Funding deposit address ${context.depositAddress} with ${topUpAmount.toString()} wei from ${sponsorAccount.address}`,
    )
    if (!args['dry-run']) {
