...
test('checkout lifecycle works with idempotency, multi chain verification, and derived manual payment state', async () => {
  ...
  const config = {
    ...
    minConfirmations: 1,
    chains: {
      ethereum: { chainId: 1, name: 'Ethereum', nativeAsset: 'ETH', rpcUrlEnv: 'RPC_ETHEREUM' },
  ...
  providers.register('ethereum', { verifyPayment: async ({ recipientAddress }) => ({ ok: true, reason: 'confirmed', confirmations: 2, recipientAddress }) });
  ...
  const priceService = {
    ...
    quoteUsd: async ({ chain }) => ({ baseUnits: chain === 'base' ? 1000000n : 1005000n, decimalAmount: chain === 'base' ? '1' : '1.005', priceUsd: 1, priceMicros: 1000000, source: 'test', fetchedAt: new Date().toISOString() })
  };
  const manualPaymentService = {
    isConfigured: () => true,
    status: () => ({ configured: true, sponsorAddress: '0x9999999999999999999999999999999999999999', derivationPath: "m/44'/60'/0'/0" }),
    createCheckoutManualPayment: async () => ({
      available: true,
      status: 'awaiting_payment',
      derivationIndex: store.find('checkouts', (checkout) => checkout.manualPayment?.available).length,
    ...
    getCheckoutDetails: async (checkout) => ({ available: true, address: checkout.manualPayment.address, qrSvg: '<svg></svg>' })
  };
  ...
  assert.equal(created.json.checkout.recipientByChain.base, '0xa9c424d119323495c3260ef16c7813a1133cd84e');
  ...
  const createdSecond = await invokeApi({
    method: 'POST',
    url: '/api/checkouts',
    body: { merchantId: 'merchant_default', orderId: 'order_2', amountUsd: 1, acceptedAssets: ['USDC', 'USDT'], enabledChains: ['ethereum', 'base'] },
    ctx
  });
  assert.equal(createdSecond.statusCode, 201);
  assert.notEqual(createdSecond.json.checkout.manualPayment.address, created.json.checkout.manualPayment.address);
  ...
  assert.equal(submit.json.payment.recipientAddress, '0xa9c424d119323495c3260ef16c7813a1133cd84e');

  const status = await invokeApi({ method: 'GET', url: `/api/checkouts/${created.json.checkout.id}/status`, ctx });
...
test('status polling auto-confirms a previously submitted pending payment', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'charge-with-crypto-api-poll-'));
  const config = {
    env: 'test',
    port: 0,
    baseUrl: 'http://127.0.0.1:0',
  ...
  providers.register('base', {
    verifyPayment: async ({ recipientAddress }) => {
      ...
      if (verificationAttempts < 2) return { ok: false, reason: 'insufficient_confirmations', confirmations: 0, recipientAddress };
      return { ok: true, reason: 'confirmed', confirmations: 2, recipientAddress };
    }
  });
  const priceService = {
    getAssetPrice: async () => ({ priceUsd: 1, priceMicros: 1000000, source: 'test', fetchedAt: new Date().toISOString() }),
  ...
  const created = await invokeApi({
    method: 'POST',
    url: '/api/checkouts',
    body: { merchantId: 'merchant_default', orderId: 'order_polling_1', amountUsd: 1, asset: 'USDC', enabledChains: ['base'] },
    ctx
  });
  assert.equal(created.statusCode, 201);
...
test('checkout can be resolved from merchant webhook using the same endpoint later used for payment events', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'charge-with-crypto-api-resolve-'));
  const config = {
    env: 'test',
    port: 0,
    baseUrl: 'http://127.0.0.1:0',
  ...
  const priceService = {
    getAssetPrice: async () => ({ priceUsd: 1, priceMicros: 1000000, source: 'test', fetchedAt: new Date().toISOString() }),
    quoteUsd: async () => ({ baseUnits: 25000000n, decimalAmount: '25', priceUsd: 1, priceMicros: 1000000, source: 'test', fetchedAt: new Date().toISOString() })
  };
  const ctx = { store, config, providers, priceService };
...
test('checkout payment rail stays isolated between bitcoin and evm routes', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'charge-with-crypto-api-rail-'));
  const config = {
    env: 'test',
    port: 0,
    baseUrl: 'http://127.0.0.1:0',
  ...
  const providers = new ProviderRegistry();
  providers.register('bitcoin', { verifyPayment: async ({ recipientAddress }) => ({ ok: true, reason: 'confirmed', confirmations: 2, recipientAddress }) });
  providers.register('base', { verifyPayment: async ({ recipientAddress }) => ({ ok: true, reason: 'confirmed', confirmations: 2, recipientAddress }) });
  const priceService = {
    getAssetPrice: async ({ asset }) => asset === 'BTC'
      ? ({ priceUsd: 60000, priceMicros: 60000000000, source: 'test', fetchedAt: new Date().toISOString() })
      : ({ priceUsd: 1, priceMicros: 1000000, source: 'test', fetchedAt: new Date().toISOString() }),
    quoteUsd: async ({ asset }) => asset === 'BTC'
      ? ({ baseUnits: 10000n, decimalAmount: '0.0001', priceUsd: 60000, priceMicros: 60000000000, source: 'test', fetchedAt: new Date().toISOString() })
      : ({ baseUnits: 6000000n, decimalAmount: '6', priceUsd: 1, priceMicros: 1000000, source: 'test', fetchedAt: new Date().toISOString() })
  };
  const ctx = { store, config, providers, priceService };
  ...
  const bitcoinCheckout = await invokeApi({
    method: 'POST',
    url: '/api/checkouts',
    body: { merchantId: 'merchant_default', amountUsd: 6, paymentRail: 'bitcoin' },
    ctx
  });
  ...
  await assert.rejects(
    invokeApi({
      method: 'POST',
      url: '/api/checkouts',
      body: { merchantId: 'merchant_default', amountUsd: 6, paymentRail: 'evm', enabledChains: ['bitcoin'], acceptedAssets: ['BTC'] },
      ctx
    }),
    /paymentRail/
  );
...
test('checkout can be created from a stored merchant plan and webhook resolve can return only the planId', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'charge-with-crypto-api-plan-'));
  const config = {
    env: 'test',
    port: 0,
    baseUrl: 'http://127.0.0.1:0',
  ...
  const priceService = {
    ...
    quoteUsd: async () => ({ baseUnits: 29000000n, decimalAmount: '29', priceUsd: 1, priceMicros: 1000000, source: 'fixed_peg', fetchedAt: new Date().toISOString() })
  };
  const ctx = { store, config, providers, priceService };
  ...
  const direct = await invokeApi({
    method: 'POST',
    url: '/api/checkouts',
    body: { merchantId: 'merchant_default', planId: 'pro', orderId: 'plan_order_1' },
    ctx
  });
  ...
  const resolved = await invokeApi({
    method: 'POST',
    url: '/api/checkouts/resolve',
    body: { merchantId: 'merchant_default', referenceId: 'customer_123', planId: 'pro' },
    ctx
  });
