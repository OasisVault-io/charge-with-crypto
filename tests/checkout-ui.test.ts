...
test('template selection stays explicit and merchant_default maps to the canonical brand label', () => {
  ...
  assert.equal(
    merchantDisplayName({ merchantId: 'merchant_default' }),
    'OasisVault',
  )
})
...
test('route scoring keeps the intended asset and chain preference order', () => {
  ...
  assert.equal(
    manualPaymentAvailable({ manualPayment: { available: true } }),
    true,
  )
  assert.equal(
    manualPaymentAvailable({ manualPayment: { available: false } }),
    false,
  )
})
