...
const { EvmVerifier } = require('../app/lib/services/chains/evm/evmVerifier')
...
test('erc20 verification matches the canonical transfer topic and confirms recipient transfers', async () => {
  ...
  const verification = await verifier.verifyPayment({
    txHash:
      '0xa997d5a5e26a7ffcc73f55f9af3226a512cf2be2df244635fc961eb409cd4740',
  ...
  assert.equal(
    verification.recipientAddress,
    '0xa0400933060322cae0c23b94e2b7cae0d9d0168b',
  )
  assert.equal(
...
test('evm verification rejects a tx submitted from a different wallet', async () => {
  ...
  assert.equal(
    verification.expectedWalletAddress,
