...
} = require('../app/lib/services/manual-payment/manualPaymentService')
const { SqliteStore } = require('../app/lib/store/sqliteStore')
const { deriveEvmDepositWallet } = require('../app/lib/utils/viemEvm.js')

const TEST_EVM_XPUB = HDKey.fromMasterSeed(Buffer.alloc(32, 7)).derive(
  "m/44'/60'/0'/0",
).publicExtendedKey
...
function createConfig() {
  return {
    ...
    manualPaymentSweepSponsorPrivateKey:
      '0x59c6995e998f97a5a0044966f0945382d7fd3b4b9f0c0d5857a1f5edecf66f7e',
    chains: {
      base: {
        chainId: 8453,
        name: 'Base',
...
test('scanChainForCheckout clamps retry log scans to the refreshed confirmed head', async () => {
  const dir = fs.mkdtempSync(
    path.join(os.tmpdir(), 'chaincart-manual-payment-'),
  )
...
test('scanChainForCheckout waits without storing an error when the refreshed head falls behind nextBlock', async () => {
  ...
  assert.equal(
    refreshed.manualPayment.scanState.base.lastScanAt.length > 0,
    true,
  )
  assert.equal('error' in refreshed.manualPayment.scanState.base, false)
})
...
test('scanChainForCheckout starts after the current head for fresh checkouts', async () => {
  const dir = fs.mkdtempSync(
    path.join(os.tmpdir(), 'chaincart-manual-payment-fresh-head-'),
  )
  const store = new SqliteStore(dir)
  const service = new ManualPaymentService({ store, config: createConfig() })
  const { checkout, quote } = seedCheckout(store, 90)
  ...
  await service.scanChainForCheckout(
    store.getById('checkouts', checkout.id),
    'base',
    [quote],
  )
...
test('createCheckoutManualPayment derives deterministic evm deposit addresses from an xpub without a mnemonic', async () => {
  const dir = fs.mkdtempSync(
    path.join(os.tmpdir(), 'chaincart-manual-payment-xpub-'),
  )
  const store = new SqliteStore(dir)
  ...
  assert.equal(manualPayment.address, await deriveExpectedAddress(0))
})

test('createCheckoutManualPayment respects the configured manual payment start index', async () => {
  const dir = fs.mkdtempSync(
    path.join(os.tmpdir(), 'chaincart-manual-payment-start-index-'),
  )
  const store = new SqliteStore(dir)
  const config = {
    ...createXpubConfig(),
  ...
  assert.equal(manualPayment.address, await deriveExpectedAddress(500000))
})

test('createCheckoutManualPayment raises an existing derivation counter to the configured start index', async () => {
  const dir = fs.mkdtempSync(
    path.join(os.tmpdir(), 'chaincart-manual-payment-counter-floor-'),
  )
  const store = new SqliteStore(dir)
  const config = {
    ...createXpubConfig(),
  ...
  assert.equal(
    store.getById('counters', 'manual_payment_derivation_index').value,
    500001,
  )
})
...
test('xpub manual pay marks detected deposits as requiring manual sweep when no sweeper is configured', async () => {
  const dir = fs.mkdtempSync(
    path.join(os.tmpdir(), 'chaincart-manual-payment-manual-sweep-'),
  )
  const store = new SqliteStore(dir)
  const config = createXpubConfig()
  const service = new ManualPaymentService({ store, config })
  ...
  const refreshed = await service.scanChainForCheckout(
    store.getById('checkouts', checkout.id),
    'base',
    [quote],
  )
  const payment = store.find(
    'payments',
    (candidate) => candidate.checkoutId === checkout.id,
  )[0]
...
test('scanChainForCheckout ignores empty hex log payloads instead of crashing on BigInt parsing', async () => {
  const dir = fs.mkdtempSync(
    path.join(os.tmpdir(), 'chaincart-manual-payment-empty-log-'),
  )
  const store = new SqliteStore(dir)
  const config = createXpubConfig()
  const service = new ManualPaymentService({ store, config })
  ...
  assert.equal(
    store.find('payments', (candidate) => candidate.checkoutId === checkout.id)
      .length,
    0,
  )
})
...
test('scanChainForCheckout ignores deposit logs from blocks older than checkout creation time', async () => {
  const dir = fs.mkdtempSync(
    path.join(os.tmpdir(), 'chaincart-manual-payment-stale-history-'),
  )
  const store = new SqliteStore(dir)
  const config = createXpubConfig()
  const service = new ManualPaymentService({ store, config })
