...
test('x402 resolve advertises Base USDC payment requirements and settles into the normal payment.confirmed flow', async () => {
  ...
  assert.equal(
    paymentRequired.accepts[0].payTo.toLowerCase(),
    '0xa9c424d119323495c3260ef16c7813a1133cd84e',
  )
  assert.equal(
    paymentRequired.accepts[0].asset.toLowerCase(),
    config.assets.USDC.addresses.base.toLowerCase(),
  )
  ...
  const confirmedEvent = store.find(
    'events',
    (event) =>
      event.checkoutId === checkout.id && event.type === 'payment.confirmed',
  )[0]
  ...
  const delivery = store.find(
    'webhook_deliveries',
    (entry) => entry.eventId === confirmedEvent.id,
  )[0]
  assert.ok(delivery)
...
test('checkout-scoped x402 route lets the same checkout be paid by an agent and later reports paid status', async () => {
  const dir = fs.mkdtempSync(
    path.join(os.tmpdir(), 'charge-with-crypto-x402-checkout-'),
  )
  const config = testConfig(dir)
  const store = new SqliteStore(dir)
  ensureMerchantDefaults(store, config)
...
test('product-scoped x402 route advertises Bazaar metadata and settles the shared product checkout flow', async () => {
  const dir = fs.mkdtempSync(
    path.join(os.tmpdir(), 'charge-with-crypto-x402-product-'),
  )
  const config = testConfig(dir)
  const store = new SqliteStore(dir)
  ensureMerchantDefaults(store, config)
  ...
  assert.equal(
    paymentRequired.extensions?.bazaar?.info?.input?.bodyType,
    'json',
  )
