...
const { getActiveQuotesForCheckout } = require('./quoteService.ts');
const { recordManualDetectedPayment } = require('./paymentService.ts');
const { formatBitcoinUri } = require('../../utils/bitcoin.ts');
...
class BitcoinManualPaymentService {
  constructor({ store, config, fetchImpl = fetch }) {
    ...
    this.client = config.chains.bitcoin ? new (require('./bitcoinClient.ts').BitcoinClient)({
