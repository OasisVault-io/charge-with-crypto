...
import { requireBitcoinXpub } from '../../utils/bitcoin'
import {
  requireEnum,
...
import {
  ...
  DEMO_MERCHANT_ID,
} from '../merchant/merchantDefaults'
import {
  type AnyRecord,
  type AppConfig,
  type MerchantLike,
...
function defaultCheckoutEnabledChainsForRail({
  merchant,
  config,
  paymentRail,
}) {
...
function defaultCheckoutAcceptedAssetsForRail({
  merchant,
  config,
  paymentRail,
}) {
...
function deriveCheckoutEnabledChains({ body, merchant, config, paymentRail }) {
  ...
  if (requested.some((chain) => chainRail(chain) !== paymentRail)) {
    throw new Error(
      'enabledChains contains chains outside selected paymentRail',
    )
  }
...
function deriveCheckoutAcceptedAssets({
  ...
  const requested =
    Array.isArray(body.acceptedAssets) && body.acceptedAssets.length
      ...
      : Array.isArray(body.assets) && body.assets.length
        ...
        : body.asset
          ? normalizeRequestedAssets([body.asset], config, 'asset')
          : defaultCheckoutAcceptedAssetsForRail({
              merchant,
              config,
              paymentRail,
            })
  ...
  if (requested.some((asset) => assetRail(asset) !== paymentRail)) {
    throw new Error(
      'acceptedAssets contains assets outside selected paymentRail',
    )
  }

  const unique = [...new Set(requested)].filter((asset) =>
    enabledChains.some((chain) =>
      isAssetSupportedOnChain(config, chain, asset),
    ),
  )
  if (!unique.length) {
    throw new Error(
...
function deriveAcceptedAssets({
  ...
  if (!unique.length) {
    throw new Error(
      'merchant has no supported accepted assets on enabled chains',
    )
  }
  return unique
}

function normalizePlans(plans: unknown, config: AppConfig) {
  const input: MerchantPlanLike[] = Array.isArray(plans)
    ? plans.filter(
        (plan): plan is MerchantPlanLike =>
  ...
  return input.map((plan, index) => {
    ...
    return {
      ...
      amountUsd: Number(
        (normalizeUsdCents(plan.amountUsd || 0) / 100).toFixed(2),
      ),
      paymentRail,
      enabledChains,
      acceptedAssets,
...
function normalizeManualPaymentEnabledChains(value, config, merchant = null) {
  ...
  return uniq(supported).filter((chain) =>
    merchantSupportsChain(merchant, chain),
  )
}
...
function normalizeMerchantPayload({
  ...
  body: AnyRecord
  merchant?: MerchantLike | null
  config: AppConfig
  nextAddresses: Record<string, string>
  nextBitcoinXpub: string
}) {
  const payload: AnyRecord = {}
  if (body.name != null) {
    payload.name =
      requireOptionalString(body.name, 'name', { max: 80 }) ||
      merchant?.name ||
      ''
  }
  ...
  if (body.supportEmail != null) {
    payload.supportEmail = requireOptionalString(
      body.supportEmail,
      'supportEmail',
      {
        max: 160,
