import { getConfig, getAppContext, priceService as legacyPriceService, legacyApi } from '../server/runtime';

export function getPublicConfig() {
  const config = getConfig();
  const context = getAppContext();
  return {
    appMode: legacyApi.configuredAppMode(config),
    chains: config.chains,
    assets: config.assets,
    fixedPriceAssets: Object.keys(config.assets).filter((asset) => legacyPriceService.isFixedPegAsset(asset)),
    ...
    manualPayment: context.manualPaymentService?.status?.() || { configured: false, sponsorAddress: '', derivationPath: '' },
...
export function getHealth() {
  ...
  const rpc = Object.fromEntries(
    Object.entries(config.chains).map(([chain, chainConfig]: [string, any]) => [chain, Boolean(process.env[chainConfig.rpcUrlEnv])])
  );
  ...
  return {
    ...
    rpcConfigured: rpc,
    storage: 'sqlite',
    manualPaymentConfigured: Boolean(context.manualPaymentService?.isConfigured?.())
  };
}
