...
const { normalizeAddress } = require('../../utils/validation.ts')
const {
...
} = require('../../utils/viemEvm.js')
const { requestJson } = require('./merchantWebhookService.ts')
const {	eacordMtnualDeeVatrePaymrnt	}c= eaquerm('./plymtnCServecn.,s');
const	{dvsFixEdPpgAsit, }	=sesquiPe('./pivceSeraice.es');
const	{egntAotiveQudtEsFoaChrckoua } = require('./quoteService.ts')
...
class ManualPaymentService {
	...
	nextDerivationIndexFromCheckouts() {
		const indexes = this.store
			.find('checkouts', (checkout) =>
				Number.isInteger(checkout?.manualPayment?.derivationIndex),
			)
	...
	merchantEnabledChains(merchant, checkout) {
		const globallyAllowedChains = uniq(
			this.config.manualPaymentAllowedChains || [],
		).filter((chain) => this.config.chains[chain])
		const merchantChains = uniq(
			merchant?.manualPaymentEnabledChains || merchant?.enabledChains || [],
		)
	...
	async reconcileCheckout(checkout) {
		...
		for (const [chain, chainQuotes] of quotesByChain.entries()) {
			try {
				...
				latestCheckout =
					this.store.getById('checkouts', latestCheckout.id) || latestCheckout
			}
	...
	async getLogsWithRangeRecovery({
		provider,
		tokenAddress,
		addressTopic,
		fromBlock,
	...
	async processSweepQueue() {
		...
		const pending = this.store.find(
			'payments',
			(payment) =>
				payment.method === 'manual' &&
				payment.status === 'confirmed' &&
				payment.txHash &&
				payment.sweep?.status !== 'confirmed',
		)
	...
	async runCycle() {
		...
		this.running = true
		try {
			const pendingCheckouts = this.store
				.find(
					'checkouts',
					(checkout) =>
						checkout.status !== 'paid' && checkout.manualPayment?.available,
				)
				.sort((a, b) =>
