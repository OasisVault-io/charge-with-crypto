class X402RequestAdapter {
	...
	checkoutRequestHandler: (request: Request, checkoutId: string) => Promise<unknown>
	productAccessRequestHandler: (request: Request, productId: string) => Promise<unknown>
	handleResolve: (req: unknown, body?: Record<string, unknown>) => Promise<unknown>
	handleCheckout: (
		req: unknown,
		checkoutId: string,
