...
function findOne<T extends AnyRecord>(
  ...
  if (store.findOne) {
    return store.findOne(collection, (item) =>
      predicate(item as unknown as T),
    ) as unknown as T | null
  }
  return (
    (store.find(collection, (item) =>
      predicate(item as unknown as T),
    )[0] as unknown as T | undefined) || null
  )
}
...
class X402Service {
  ...
  purchaseKeyMatchesSale(
    ...
    return this.settlement.purchaseKeyMatchesSale(checkout, sale, resourcePath)
  }
