...
const { requireEnum, requireOptionalString } = require('../../utils/validation.ts')
const { DEMO_MERCHANT_ID } = require('./merchantDefaults.ts')
const {
	CHECKOUT_TEMPLATES,
...
function normalizeProductTags(value) {
	...
	return [...new Set(value.map((tag) => String(tag || '').trim().toLowerCase()).filter(Boolean))].slice(
		0,
		12,
	)
}
...
function normalizeProductPayload({ body, existing, store, config }) {
	...
	const paymentRail = derivePaymentRail({
		body: productInput,
		fallback:
			existing?.paymentRail || plan?.paymentRail || merchant.defaultPaymentRail || 'evm',
		config,
	})
