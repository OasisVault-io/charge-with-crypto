// @ts-nocheck
const { normalizeProductId, publicProduct, productEndpoints, requireProduct, resolveProductMerchant, buildProductCheckoutInput, upsertManagedProducts } = require('../services/catalog/productService.ts');
const { resolveCheckoutFromMerchant } = require('../services/merchant/merchantWebhookClient.ts');
const { verifyPaymentAndRecord, reconcilePendingCheckoutPayments } = require('../services/payments/paymentService.ts');
const { createQuote, getActiveQuote, getActiveQuotesForCheckout, getQuoteById, getLatestQuoteForSelection } = require('../services/payments/quoteService.ts');
const { isFixedPegAsset } = require('../services/pricing/priceSupport.ts');
const { normalizeUsdCents } = require('../utils/amounts.ts');
const { requireBitcoinXpub } = require('../utils/bitcoin.ts');
const { parseJsonBody, sendJson } = require('../utils/http.ts');
const { randomId } = require('../utils/id.ts');
const { requireChainAddress, requireEnum, requireOptionalString, requireUrl } = require('../utils/validation.ts');
...
function merchantSupportsChain(merchant, chain) {
  if (chain === 'bitcoin') return Boolean(merchant?.recipientAddresses?.bitcoin || merchant?.bitcoinXpub);
...
function ensureMerchantDefaults(store, config) {
  ...
  for (const definition of merchantDefinitions) {
    ...
    if (typeof existing.publicCheckoutAllowed !== 'boolean') patch.publicCheckoutAllowed = definition.publicCheckoutAllowed;
...
function deriveEnabledChains({ body, merchant, config }) {
  ...
  const unique = [...new Set(requested.map((chain) => requireEnum(chain, Object.keys(config.chains), 'enabledChains')))].filter(
    (chain) => merchantSupportsChain(merchant, chain)
  );
...
function normalizeMerchantPayload({ body, merchant, config, nextAddresses, nextBitcoinXpub }) {
  ...
  if (body.publicCheckoutAllowed != null) payload.publicCheckoutAllowed = Boolean(body.publicCheckoutAllowed);
...
function normalizeProductPayload({ body, existing, store, config }) {
  ...
  const checkoutTemplate = requireEnum(
    body.checkoutTemplate || body.template || existing?.checkoutTemplate || (merchantId === DEMO_MERCHANT_ID ? 'neutral' : 'oasis'),
    CHECKOUT_TEMPLATES,
    'checkoutTemplate'
  );
...
async function createCheckoutResponse({ store, config, priceService, manualPaymentService, bitcoinAddressService, body, createManualPayment = true }) {
  ...
  if (!Number.isInteger(quantity) || quantity <= 0) return { status: 400, body: { error: 'quantity must be a positive integer' } };

  const amountUsd = Number((normalizeUsdCents(checkoutInput.amountUsd || 0) / 100).toFixed(2));
  if (amountUsd <= 0) return { status: 400, body: { error: 'amountUsd must be greater than zero' } };
  const paymentRail = derivePaymentRail({
    body: checkoutInput,
  ...
  const recipientByChain = Object.fromEntries(enabledChains.map((chain) => {
    ...
    if (chain === 'bitcoin' && bitcoinSettlement?.address) return [chain, bitcoinSettlement.address];
...
function resolveQuoteForSubmission(store, checkout, body) {
  if (body.quoteId) {
    ...
    if (quote && (!quote.expiresAt || new Date(quote.expiresAt).getTime() > Date.now())) return quote;
  }
...
async function handleApi(req, res, ctx) {
  ...
  if (req.method === 'GET' && path === '/mcp') {
    if (!mcpService) return sendJson(res, 503, { error: 'mcp_unavailable', message: 'MCP is not configured.' });
  ...
  if (req.method === 'POST' && path === '/api/x402/resolve') {
    if (!x402Service) return sendJson(res, 503, { error: 'x402_unavailable', message: 'x402 agent payments are not configured.' });
    try {
  ...
  if (req.method === 'POST' && path === '/api/products') {
    ...
    if (store.getById('products', id)) return sendJson(res, 409, { error: 'product already exists' });
  ...
  if (req.method === 'POST' && path === '/api/merchants') {
    ...
    for (const chain of Object.keys(config.chains)) {
      ...
      if (value) recipientAddresses[chain] = requireChainAddress(value, chain, `${chain} recipientAddress`, config.chains[chain]);
    }
  ...
  if (req.method === 'POST' && path === '/api/checkouts') {
    ...
    if (access) return sendJson(res, access.status, access.body);
  ...
  if (req.method === 'POST' && balanceScanMatch) {
    ...
    if (!scopedQuotes.length) return sendJson(res, 400, { error: 'no routes available for selected wallet rail' });
