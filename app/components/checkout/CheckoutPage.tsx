...
export function CheckoutPage({ checkoutId, initialData, templateParam = '' }: CheckoutPageProps) {
  ...
  const [logoSrc, setLogoSrc] = useState(safeLogoSrc(initialData?.checkout?.merchantLogoUrl));
  ...
  const paymentRail = checkout?.paymentRail === 'bitcoin'
    ? 'bitcoin'
  ...
  const manualRoutes = viewState?.manualDetails?.routes || checkout?.manualPayment?.routes || [];
  const selectedManualRoute = manualRoutes.find((route: any) => route.key === manualSelectedRouteKey)
    || manualRoutes.find((route: any) => route.key === viewState?.manualDetails?.preferredRouteKey)
    || manualRoutes[0]
    || null;
  ...
  const walletCompatibleQuotes = useMemo(() => {
    ...
    return activeQuotes(quotes, now).filter((quote) => walletSession.rail === 'bitcoin' ? quote.chain === 'bitcoin' : quote.chain !== 'bitcoin');
  }, [walletSession, quotes, now]);

  const recommendedQuote = useMemo(() => {
    if (!walletSession) return null;
    const payable = walletCompatibleQuotes.filter((quote) => isQuotePayable(viewState?.balances, quote));
    const ranked = (payable.length ? payable : walletCompatibleQuotes).slice().sort((a, b) => routeScore(a) - routeScore(b));
  ...
  const availableWalletProviders = useMemo(() => {
    ...
    const hasBitcoin = liveQuotes.some((quote) => quote.chain === 'bitcoin' && quote.asset === 'BTC');
    const hasEvm = liveQuotes.some((quote) => quote.chain !== 'bitcoin');
    if (paymentRail === 'evm' && hasEvm) next.push({ id: 'injected-evm', label: 'Injected EVM', rail: 'evm' });
    if (paymentRail === 'bitcoin' && hasBitcoin) next.push({ id: 'xverse', label: 'Xverse BTC', rail: 'bitcoin' });
    return next;
  ...
  }, [checkout?.status, manualPanelOpen, checkout, pendingPayment, refreshStatus, payments, quotes]);

  async function loadManualDetails() {
    if (!manualPaymentAvailable(checkout)) return null;
    const details = await fetchJson<any>(`/api/checkouts/${checkoutId}/manual-payment`);
    setViewState((current: any) => ({
      ...current,
      manualDetails: details
    }));
    setManualSelectedRouteKey((current) => current || details.preferredRouteKey || '');
    return details;
  }
  ...
  async function copyManualAddress() {
    const address = selectedManualRoute?.address || viewState?.manualDetails?.address || checkout?.manualPayment?.address;
    if (!address) {
  ...
  async function connectWallet(providerId = availableWalletProviders[0]?.id || '') {
    ...
    try {
      ...
      if (providerId === 'xverse') {
        ...
        let accounts = Array.isArray(response?.result?.addresses) ? response.result.addresses : Array.isArray(response?.result) ? response.result : [];
        if (!accounts.length && xverse.AddressPurpose) {
          response = await xverse.request('getAccounts', {
            purposes: [xverse.AddressPurpose.Payment],
            message: 'Charge With Crypto wants to connect your Bitcoin payment address.'
          });
          accounts = Array.isArray(response?.result?.addresses) ? response.result.addresses : Array.isArray(response?.result) ? response.result : [];
        }
        if (response?.status === 'error') {
          throw new Error(response?.error?.message || 'xverse address request failed');
        }
        const paymentAccount = accounts.find((account: any) => String(account.purpose).toLowerCase() === 'payment') || accounts[0];
        if (!paymentAccount?.address) throw new Error('xverse did not return a payment address');
        session = { rail: 'bitcoin', provider: 'xverse', address: paymentAccount.address };
      } else {
  ...
  async function refreshQuote() {
    ...
    try {
      ...
      const refreshed = await fetchJson<any>(
        ...
        jsonRequest(quote ? { chain: quote.chain, asset: quote.asset } : {}, { method: 'POST' })
      );
      setViewState((current: any) => {
        const byRoute = new Map((current?.quotes || []).map((entry: any) => [quoteKey(entry), entry]));
  ...
  async function submitTx(txHash: string, walletAddress: string, quote: any) {
    await fetchJson(
      ...
      jsonRequest(
        { txHash, walletAddress, chain: quote.chain, asset: quote.asset, quoteId: quote.id },
        { method: 'POST' }
      )
    );
    await refreshStatus();
    ...
    try {
      if (walletSession?.provider === 'xverse') {
        ...
        const response = await xverse.request('sendTransfer', {
          recipients: [
            {
              address: checkout?.recipientByChain?.bitcoin || checkout?.recipientAddress,
              amount
            }
          ]
        });
        ...
        const recipientAddress = checkout?.recipientByChain?.[recommendedQuote.chain] || checkout?.recipientAddress;
        ...
        if (recommendedQuote.asset === 'ETH') {
          ...
          const tokenAddress = config.assets?.[recommendedQuote.asset]?.addresses?.[recommendedQuote.chain];
          const methodId = '0xa9059cbb';
          const toArg = recipientAddress.toLowerCase().replace(/^0x/, '').padStart(64, '0');
          const amountArg = BigInt(recommendedQuote.cryptoAmountBaseUnits).toString(16).padStart(64, '0');
          txHash = await window.ethereum.request({
            method: 'eth_sendTransaction',
            params: [{
              from: walletAddress,
  ...
  const routeBalance = recommendedQuote ? balanceForQuote(viewState?.balances, recommendedQuote) : null;
  const routePayable = recommendedQuote ? isQuotePayable(viewState?.balances, recommendedQuote) : false;
          ...
          <div className="brand-copy">
            <h1 className="brand-title">
              ...
              <span className="brand-line brand-line-accent">
                <img alt="" className="brand-isotype" src="/oasisvault-isotype-green.png" />
                <span>{heroParts.lead}</span>
              </span>
              <span className="brand-line" hidden={!heroParts.trail}>{heroParts.trail}</span>
            </h1>
            <p className="brand-description">{checkout?.description || 'Connect a wallet and Charge With Crypto will recommend the cleanest way to pay.'}</p>
          </div>
        </section>
          ...
          <section className="payment-card">
            <header className="payment-card-header">
              ...
              <div className="hero-total">
                ...
                <div className={dueAmountClassName(amountText)} title={amountText}>{amountText}</div>
                <div className="pill-row">
                  {(checkout?.acceptedAssets || [checkout?.asset]).map((asset: string) => (
                    <span className="asset-pill" key={asset}>{asset}</span>
                  ))}
                </div>
              </div>
            </header>
            ...
            {checkout?.status === 'paid' ? (
              <section className="success-card">
                ...
                <div className="success-meta">
                  ...
                  <div className="success-meta-row">
                    <span>Asset</span>
                    <strong>{payments.find((payment: any) => payment.status === 'confirmed')?.asset || checkout?.paidAsset || checkout?.defaultAsset || checkout?.asset}</strong>
                  </div>
                  <div className="success-meta-row">
                    <span>Network</span>
                    <strong>{chainLabel(config, payments.find((payment: any) => payment.status === 'confirmed')?.chain || checkout?.paidChain || checkout?.defaultChain)}</strong>
                  </div>
                  {payments.find((payment: any) => payment.status === 'confirmed')?.txHash ? (
                    <div className="success-meta-row">
                      <span>Transaction</span>
                      <strong>
                        <a
                          href={explorerTx(
                            payments.find((payment: any) => payment.status === 'confirmed')?.chain || checkout?.paidChain || checkout?.defaultChain,
                            payments.find((payment: any) => payment.status === 'confirmed')?.txHash
                          )}
                          rel="noreferrer"
                          target="_blank"
                        >
                          {payments.find((payment: any) => payment.status === 'confirmed')?.txHash.slice(0, 10)}...
                        </a>
                      </strong>
                    </div>
                  ) : null}
                ...
                <div className="row">
                  {checkout?.successUrl ? <a className="link-button" href={checkout.successUrl}>Return to merchant</a> : null}
                </div>
              </section>
            ) : (
              <div className="payment-flow">
                <div className={`card wallet-stage ${walletConnected ? 'wallet-connected wallet-stage-compact' : ''}`} hidden={manualPanelOpen}>
                  <div className="stack-sm">
                    <span className="eyebrow" hidden={walletConnected}>Step 1</span>
                    <h2 hidden={walletConnected}>Connect a wallet</h2>
                    <p className="muted">{walletMetaCopy}</p>
                    {walletConnected ? (
                      <div className="wallet-connected-copy">
                        <div className="code wallet-connected-address">{walletSession.address}</div>
                        <button className="tiny-link wallet-disconnect" type="button" onClick={disconnectWallet}>Disconnect</button>
                      </div>
                    ) : null}
                    {walletConnected && walletCompatibleQuotes.length ? (
                      <p className="muted">
                        {walletSession.rail === 'bitcoin' ? 'Bitcoin rail' : 'EVM rail'}
                        {' · '}
                        Scanned {[...new Set(walletCompatibleQuotes.map((quote: any) => chainLabel(config, quote.chain)))].join(', ')}
                      </p>
                    ) : null}
                  </div>
                  <div className="wallet-actions-stack">
                    {!walletConnected && paymentRail !== 'bitcoin' ? (
                      <button className="primary-button" disabled={isBusy || expired} type="button" onClick={() => void connectWallet()}>
                        {isBusy ? 'Connecting…' : 'Connect wallet'}
                      </button>
                    ) : null}
                    {!walletConnected && (paymentRail === 'bitcoin' || availableWalletProviders.length > 1) ? (
                    ...
                    {!walletConnected && manualPaymentAvailable(checkout) ? (
                      <button className="tiny-link" hidden={manualPanelOpen} type="button" onClick={() => void openManualPanel()}>
                ...
                <section className="card manual-panel" hidden={!manualPanelOpen || !manualPaymentAvailable(checkout) || expired}>
                  <div className="manual-panel-head">
                    <div className="stack-sm">
                      ...
                      <button className="tiny-link manual-switch-link" type="button" onClick={() => setManualPanelOpen(false)}>
                        Have a hot wallet? Connect it instead
                      </button>
                    </div>
                    <span className={`badge ${checkout?.status === 'paid' ? 'ok' : 'warn'}`}>
                      {checkout?.status === 'paid'
                        ? 'Payment found'
                        : selectedManualRoute?.chain === 'bitcoin'
                          ? 'Watching Bitcoin'
                  ...
                  <div className="manual-grid">
                    <div
                      ...
                      dangerouslySetInnerHTML={{ __html: selectedManualRoute?.qrSvg || '<div class="muted">QR unavailable</div>' }}
                    />
                    <div className="manual-copy">
                      ...
                      <div className="manual-address-block">
                        ...
                        <div className="code manual-address">{selectedManualRoute?.address || 'Address unavailable'}</div>
                        <button className="secondary-button" type="button" onClick={() => void copyManualAddress()}>
                          Copy address
                        </button>
                        <p className="manual-address-note muted">Demo note: in production, set <code>MANUAL_PAYMENT_XPUB</code> so each checkout gets a unique deposit address automatically.</p>
                      </div>
                      <div>
                        <span className="eyebrow">Accepted assets</span>
                        <div className="pill-row">
                          {(selectedManualRoute?.acceptedAssets || []).map((asset: string) => (
                            <span className="asset-pill" key={asset}>{asset}</span>
                          ))}
                        </div>
                      </div>
                      <div>
                        ...
                        <div className="pill-row">
                          {(selectedManualRoute?.enabledChains || []).map((chain: string) => (
                            <span className="asset-pill" key={chain}>{chainLabel(config, chain)}</span>
                          ))}
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="manual-status muted">
                    {checkout?.status === 'paid'
                      ? (() => {
                        const payment = payments.find((entry: any) => entry.status === 'confirmed');
                        return payment?.txHash
                          ? <span>Payment confirmed on {chainLabel(config, payment.chain)}. <a href={explorerTx(payment.chain, payment.txHash)} rel="noreferrer" target="_blank">View transaction</a></span>
                ...
                <section className="card route-stage" hidden={!walletConnected || manualPanelOpen}>
                  <div className="recommendation">
                    {recommendedQuote ? (
                      <div className="route-heading">
                        ...
                        <span className={`badge ${routePayable ? 'ok' : 'warn'}`}>{routePayable ? 'Ready to pay' : 'Balance too low'}</span>
                      </div>
                    ) : (
                    ...
                    {recommendedQuote ? (
                      <div className="route-card recommended-route-card">
                        <div className="route-card-main">
                          ...
                          <div className="route-amount">{recommendedQuote.cryptoAmount} {recommendedQuote.asset}</div>
                          <p className="muted">
                            {routePayable
                        ...
                        <div className="route-side">
                          ...
                          <strong className="balance-value">{routeBalance?.display ? `${routeBalance.display} ${recommendedQuote.asset}` : routeBalance?.error || 'Unavailable'}</strong>
                          <span className={`badge ${routePayable ? 'ok' : 'warn'}`}>{routePayable ? 'Payable now' : 'Needs more balance'}</span>
                        </div>
                      </div>
                    ) : (
                      <div className="route-card">
                        <span className="badge warn">Route unavailable</span>
                        <span className="muted">No compatible route was found for the connected wallet.</span>
                      </div>
                    )}
                  </div>
                </section>
                ...
                <div className="pay-actions" hidden={!walletConnected || manualPanelOpen}>
                  <button
                    ...
                    disabled={isBusy || !recommendedQuote || !routePayable || checkout?.status === 'paid'}
                    type="button"
                    onClick={() => void payWithWallet()}
                  >
                    {recommendedQuote ? `Pay ${recommendedQuote.cryptoAmount} ${recommendedQuote.asset}` : 'Pay'}
                  </button>
                  <button
                ...
                <section className="card checkout-info-card">
                  <div className="summary-panel muted">
                    <div className="summary-brand">
                      <LogoImage alt={`${merchantName} logo`} className="summary-logo" src={logoSrc} setLogoSrc={setLogoSrc} />
                ...
                <div className="status-inline muted" hidden={!statusMessage && !pendingPayment && !expired}>
                  {checkout?.status === 'paid' ? null : pendingPayment ? (
                    <div className="status-card">
                      <span className="badge warn">Payment submitted</span>
                      <div className="status-copy">Confirming on {chainLabel(config, pendingPayment.chain || checkout?.defaultChain)}.</div>
                      {pendingPayment.txHash ? (
                        <a href={explorerTx(pendingPayment.chain || checkout?.defaultChain, pendingPayment.txHash)} rel="noreferrer" target="_blank">
                          View transaction
                        </a>
                      ) : null}
                    </div>
                    ...
                    <div className="status-card">
                      ...
                      <button className="secondary-button" type="button" onClick={() => void refreshQuote()}>Refresh prices</button>
                    </div>
                  ) : statusMessage ? (
                    <div className="status-card">
...
function LogoImage({
  ...
  return <img alt={alt} className={className} src={src} onError={() => setLogoSrc(defaultMerchantLogo)} />;
}
