export const preferredChainOrder = ['bitcoin', 'base', 'arbitrum', 'polygon', 'ethereum'];
...
export function quoteKey(quote: any) {
  return `${quote.chain}:${quote.asset}`;
}

export function quoteExpiresAtMs(quote: any) {
  if (!quote?.expiresAt) return null;
  const value = new Date(quote.expiresAt).getTime();
  return Number.isFinite(value) ? value : null;
}

export function quoteExpiredLocally(quote: any, now = Date.now()) {
  const expiresAt = quoteExpiresAtMs(quote);
  return expiresAt != null && expiresAt <= now;
}

export function activeQuotes(quotes: any[], now = Date.now()) {
  return (quotes || []).filter((quote) => !quoteExpiredLocally(quote, now));
}

export function pendingOnchainPayment(payments: any[]) {
  return (payments || []).find((payment) => payment.method === 'onchain' && payment.status !== 'confirmed' && payment.txHash) || null;
}

export function paymentWindowExpired({ checkout, quotes, payments }: { checkout: any; quotes: any[]; payments: any[] }, now = Date.now()) {
  if (checkout?.status === 'paid' || pendingOnchainPayment(payments)) return false;
  const expiringQuotes = (quotes || []).filter((quote) => quoteExpiresAtMs(quote) != null);
  if (!expiringQuotes.length) return false;
  return expiringQuotes.every((quote) => quoteExpiredLocally(quote, now));
}

export function paymentWindowRemainingMs(quotes: any[], now = Date.now()) {
...
export function merchantDisplayName(checkout: any) {
...
export function merchantHeroParts(checkout: any) {
  const merchantName = merchantDisplayName(checkout).replace(/\s+checkout$/i, '').trim();
  if (/^oasis\s*vault$/i.test(merchantName) || /^oasisvault$/i.test(merchantName)) {
...
export function checkoutTemplate(checkout: any, templateParam = '') {
  const candidate = (templateParam || checkout?.checkoutTemplate || '').toLowerCase();
  return candidate === 'oasis' ? 'oasis' : 'neutral';
}

export function balanceForQuote(balances: Record<string, any> | undefined, quote: any) {
  return balances?.[quoteKey(quote)] || null;
}

export function isQuotePayable(balances: Record<string, any> | undefined, quote: any) {
...
export function routeScore(quote: any) {
...
export function manualPaymentAvailable(checkout: any) {
  return Boolean(checkout?.manualPayment?.available);
}

export function refreshNeeded(config: any, checkout: any) {
