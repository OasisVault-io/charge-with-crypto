  ...
  switch (action.type) {
    case 'hydrate':
      return {
        ...
        merchantDraft: createMerchantDraft(action.initialData?.merchant, action.appConfig),
        checkoutDraft: createCheckoutDraft(action.initialData?.merchant, action.appConfig)
      };
    case 'dashboard_reloaded':
      return {
        ...state,
        dashboardData: action.next,
        merchantDraft: createMerchantDraft(action.next?.merchant, action.appConfig),
        checkoutDraft: createCheckoutDraft(action.next?.merchant, action.appConfig),
        createdCheckout: null
      };
...
export const useDashboardPageState = (
  ...
  return {
    state,
    hydrate(initialData: DashboardData, nextAppConfig: DashboardConfig | null | undefined) {
      dispatch({ type: 'hydrate', initialData, appConfig: nextAppConfig });
    },
    dashboardReloaded(next: DashboardData, nextAppConfig: DashboardConfig | null | undefined) {
    ...
    setMerchantField<K extends keyof DashboardMerchantDraft>(field: K, value: DashboardMerchantDraft[K]) {
      setMerchantDraft({
        ...state.merchantDraft,
        [field]: value
      });
    ...
    setMerchantDefaultRail(nextRail: string) {
      ...
      const nextAssets = valuesForRail(state.merchantDraft.defaultAcceptedAssets, normalizedRail, 'asset');
      setMerchantDraft({
        ...state.merchantDraft,
    ...
    setCheckoutField<K extends keyof DashboardCheckoutDraft>(field: K, value: DashboardCheckoutDraft[K]) {
      setCheckoutDraft({
        ...state.checkoutDraft,
        [field]: value
      });
    ...
    syncCheckoutRail(nextRail: string) {
      const merchantChainDefaults = valuesForRail(state.merchantDraft.enabledChains, nextRail, 'chain');
      const merchantAssetDefaults = valuesForRail(state.merchantDraft.defaultAcceptedAssets, nextRail, 'asset');
      const enabledChains = valuesForRail(state.checkoutDraft.enabledChains, nextRail, 'chain');
      const acceptedAssets = valuesForRail(state.checkoutDraft.acceptedAssets, nextRail, 'asset');
      setCheckoutDraft({
        ...state.checkoutDraft,
    ...
    updatePlan(index: number, patch: Partial<DashboardMerchantDraft['plans'][number]>) {
    ...
    removePlan(index: number) {
      setMerchantDraft({
        ...state.merchantDraft,
        plans: state.merchantDraft.plans.filter((_, planIndex) => planIndex !== index)
      });
      dispatch({
        type: 'expanded_plan_index_set',
        value:
    ...
    applySelectedPlan(planId: string) {
      const plan = state.merchantDraft.plans.find((entry) => entry.id === planId);
      if (!plan) {
        setCheckoutDraft({
          ...state.checkoutDraft,
          planId
