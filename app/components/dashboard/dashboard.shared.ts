...
export type DashboardSection = typeof setupItems[number][0] | typeof runItems[number][0];
...
export function normalizePaymentRail(value: string | undefined, fallback = 'evm') {
...
export function valuesForRail(values: string[] | undefined, rail: string, kind: 'chain' | 'asset') {
  const matcher = kind === 'chain' ? chainRail : assetRail;
  return (values || []).filter((value) => matcher(value) === rail);
}

export function defaultRailSelection(config: any, rail: string, kind: 'chain' | 'asset') {
  if (kind === 'chain') {
    const chains = Object.keys(config?.chains || {}).filter((chain) => chainRail(chain) === rail);
    if (rail === 'bitcoin') return chains.slice(0, 1);
    return chains.includes('base') ? ['base'] : chains.slice(0, 1);
  }

  const assets = Object.keys(config?.assets || {}).filter((asset) => assetRail(asset) === rail);
...
export function chainLabel(config: any, chain: string) {
...
export function merchantSettlementConfigured(merchant: any, chain: string) {
  return chain === 'bitcoin'
    ? Boolean(merchant?.recipientAddresses?.bitcoin || merchant?.bitcoinXpub)
    : Boolean(merchant?.recipientAddresses?.[chain]);
}

export function merchantPayments(data: any) {
  const checkoutMap = new Map((data?.checkouts || []).map((checkout: any) => [checkout.id, checkout]));
  return (data?.payments || [])
    .filter((payment: any) => checkoutMap.has(payment.checkoutId))
    .map((payment: any) => ({ ...payment, checkout: checkoutMap.get(payment.checkoutId) }))
    .sort((a: any, b: any) => String(b.createdAt || '').localeCompare(String(a.createdAt || '')));
}

export function filteredMerchantPayments(data: any, paymentFilterState: PaymentFilterState) {
...
export function newPlan(config: any) {
  return {
...
export function createMerchantDraft(merchant: any, config: any) {
  ...
  return {
    ...
    enabledChains,
    defaultAcceptedAssets: merchant?.defaultAcceptedAssets || defaultRailSelection(config, defaultPaymentRail, 'asset'),
    recipientAddresses: { ...(merchant?.recipientAddresses || {}) },
    manualPaymentEnabledChains: manualEnabled,
    plans: (merchant?.plans || []).map((plan: any) => ({
      id: plan.id || '',
...
export function createCheckoutDraft(merchant: any, config: any) {
  const rail = normalizePaymentRail(merchant?.defaultPaymentRail);
  const enabledChains = valuesForRail(merchant?.enabledChains || [], rail, 'chain');
  const acceptedAssets = valuesForRail(merchant?.defaultAcceptedAssets || [], rail, 'asset');
  return {
    planId: '',
    title: 'Pro plan',
    description: merchant?.checkoutDescription || 'Access unlocks after onchain confirmation.',
    paymentRail: rail,
    orderId: '',
    amountUsd: '49.00',
    referenceId: '',
    successUrl: '',
    cancelUrl: '',
    enabledChains: enabledChains.length ? enabledChains : defaultRailSelection(config, rail, 'chain'),
    acceptedAssets: acceptedAssets.length ? acceptedAssets : defaultRailSelection(config, rail, 'asset')
  };
}
