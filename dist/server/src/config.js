...
exports.projectRoot = exports.config = void 0;
exports.normalizeAppMode = normalizeAppMode;
...
const projectRoot = projectRootCandidates.find((candidate) => node_fs_1.default.existsSync(node_path_1.default.join(candidate, 'config', 'chains.json'))) || process.cwd();
exports.projectRoot = projectRoot;
loadEnv(node_path_1.default.join(projectRoot, '.env'));
const chainsConfigPath = node_path_1.default.join(projectRoot, 'config', 'chains.json');
const chainsConfig = JSON.parse(node_fs_1.default.readFileSync(chainsConfigPath, 'utf8'));
const stringOrDefault = (fallback) => zod_1.z.string().trim().default(fallback);
const urlString = (fallback) => zod_1.z.string().trim().url().default(fallback);
const optionalTrimmed = () => zod_1.z.string().trim().optional().transform((value) => value || '');
const integerDefault = (fallback) => zod_1.z.coerce.number().int().default(fallback);
const csvString = (fallback) => zod_1.z.string().trim().default(fallback).transform((value) => [...new Set(String(value || '')
        .split(',')
        .map((entry) => entry.trim().toLowerCase())
        .filter(Boolean))]);
const booleanish = (fallback = false) => zod_1.z.preprocess((value) => {
    if (value == null || value === '')
        return fallback;
...
const rawEnvSchema = zod_1.z.object({
    ...
    APP_MODE: zod_1.z.string().trim().optional(),
    PORT: integerDefault(3000),
    BASE_URL: urlString('http://localhost:3000'),
    DATA_DIR: zod_1.z.string().trim().optional(),
...
const config = {
    ...
    dataDir: parsedEnv.DATA_DIR || node_path_1.default.join(projectRoot, 'data'),
