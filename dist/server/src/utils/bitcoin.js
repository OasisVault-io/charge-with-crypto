...
function bitcoinScriptTypeForXpub(xpub) {
    const prefix = String(xpub || '').slice(0, 4).toLowerCase();
...
function requireBitcoinAddress(value, field = 'bitcoinAddress', chainConfig = {}) {
...
function deriveBitcoinAddress({ xpub, index, chainConfig = {} }) {
    const network = bitcoinNetwork(chainConfig);
    const node = bip32.fromBase58(requireBitcoinXpub(xpub, 'bitcoinXpub', chainConfig), bitcoinBip32NetworkForXpub(xpub, chainConfig));
...
function formatBitcoinUri({ address: recipientAddress, amountBtc }) {
    ...
    if (!amount)
        return `bitcoin:${recipientAddress}`;
