          ...
          <div class="field-stack">
            <div>
              ...
              <div class="checkbox-grid">
                ${Object.entries(appConfig.chains).map(([chain, chainConfig]) => planCheckboxMarkup({
                  group: `chains_${index}`,
                  value: chain,
            ...
            <div>
              ...
              <div class="checkbox-grid">
                ${Object.keys(appConfig.assets).map((asset) => planCheckboxMarkup({
                  group: `assets_${index}`,
                  value: asset,
                  label: asset,
...
function renderCheckoutPlanSummary() {
  ...
  node.innerHTML = `
    ...
    <span class="muted">${formatUsd(plan.amountUsd)} · ${escapeHtml((plan.acceptedAssets || []).join(' · ') || 'No assets')} · ${escapeHtml((plan.enabledChains || []).map((chain) => chainLabel(chain)).join(' · ') || 'No networks')}</span>
  `;
}
...
function applySelectedPlanToCheckoutForm() {
  ...
  setCheckedValues('checkoutChains', plan.enabledChains || []);
  setCheckedValues('checkoutAssets', plan.acceptedAssets || []);
}
  ...
  document.getElementById('merchantChainSettings').innerHTML = Object.entries(config.chains).map(([chain, chainConfig], index) => `
    <details class="chain-setting-card" ${index === 0 ? 'open' : ''}>
      ...
      <div class="chain-setting-body field-stack">
        <label>
          Receiving address
          <input id="recipient_${chain}" placeholder="0x..." />
        </label>
...
function setupChecklistItems(data) {
  ...
  const configuredRecipients = enabledChains.filter((chain) => merchant.recipientAddresses?.[chain]).length;
...
function renderOperationsSummary(data) {
  ...
  const merchant = data.merchant || {};
  const enabledChains = merchant.enabledChains || [];
  const configuredRecipients = enabledChains.filter((chain) => merchant.recipientAddresses?.[chain]);
...
function hydrateMerchantForm(merchant) {
  ...
  setCheckedValues('checkoutChains', merchant.enabledChains || []);
  setCheckedValues('checkoutAssets', merchant.defaultAcceptedAssets || ['USDC', 'USDT']);
...
function showCreatedCheckout(created) {
  document.getElementById('createdCheckout').innerHTML = `
    <div class="dashboard-inline-result">
      ...
      <span class="muted">${escapeHtml(created.checkout.title || created.checkout.orderId)} · ${formatUsd(created.checkout.amountUsd)}</span>
    </div>
  `;
}
